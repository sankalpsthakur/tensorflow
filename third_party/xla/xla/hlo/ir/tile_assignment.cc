...
std::optional<TileAssignment> TryToConvertV1ToV2(const TileAssignment& T) {
  if (T.iota().has_value()) {
    return T;
  }
  ...
  if (matches_iota.ok()) {
    TileAssignment transposed_V2(transposed_dims);
    std::vector<int> inv_perm(rank);
    for (int i = 0; i < rank; ++i) {
      inv_perm[perm[i]] = i;
    }
    return transposed_V2.Transpose(inv_perm);
  }
...
std::optional<AnalyzeTileAssignmentResult> AnalyzeTileAssignment(
  ...
  if (!tile_assignment.iota()) {
    ...
    if (auto v2_tile = TryToConvertV1ToV2(tile_assignment)) {
      const IotaTileAssignment& iota = *v2_tile->iota();
      if (auto sub_dims =
